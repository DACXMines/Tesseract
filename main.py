...
def main(config) :
    ...
    for i in range (nb_clients) :
        client_models.append(ClientModel(config, "client_" + str(i)))
    ...
    for epoch in range(n_epochs):
        ...
        for client_id in range(nb_clients):
            ...
            loss = criterion(output, client_train_data[client_id][minibatch])
            loss.backward()
            optimizer.step()
                    
            #append all gradients in a list
            grad_list.append([(x-y).detach() for x, y in zip(local_model.parameters(), global_model.parameters()) if x.requires_grad != 'null'])

            torch.cuda.empty_cache()
        
        #Attack
        grad_list = attack.attack_grads(grad_list)

        #Aggregation
        aggregation = DefenseSystem(grad_list, lr, cmax)
        global_model = aggregation.aggregate(config, global_model)
        torch.cuda.empty_cache()
