...
class Attack :
    ...
    def attack_grads (self, params) :
        if self.attack_type == "benign" or  self.attack_type == "data_poisoning":
            return params
        elif self.attack_type == "full_trim" :
            return full_trim_attack(self.lr, params, self.cmax)
        elif self.attack_type == "full_krum" :
            return full_krum_attack(params, self.cmax)
...
def full_trim_attack(lr, params, c_max):
    ...
    min_param = torch.min(-params, axis=0)[0]
    max_param = torch.max(-params, axis=0)[0]
    param_directions = (direction > 0) * min_param + (direction < 0) * max_param
    for i in range(c_max) :
        noise = 1 + torch.rand(params.shape[1])
...
def lambda_max(c, params):
    ...
    for i in range(n):
        for j in range(i):
            ...
            dist[j][i] = dist[i][j]
    sane_dist = torch.sort(dist[c:][c:])
    sum_benign_dist = torch.sum((sane_dist[0][:, :(n-c-1)])**2, axis=1)
    min_sane_dist = torch.min(sum_benign_dist).item()
    global_dist = torch.zeros(n-c)
...
def find_lambda(current_lambda, s, params, c):
    ...
    for i in range(1,c):
        final_params[i] = deepcopy(final_params[0])
